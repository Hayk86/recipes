...
#'
#' `step_scale_except_zero` creates a *specification* of a recipe
...
#'
step_scale_except_zero <- function(
  recipe, ...,
  role = NA,
  trained = FALSE,
  rm = 0,
...
prep.step_scale_except_zero <- function(x, training, info = NULL, ...) {
...
bake.step_scale_except_zero <- function(object, new_data, ...) {

  col_names <- object$columns

  for (i in seq_along(col_names)) {
    col <- new_data[[ col_names[i] ]]
    to_scale <- col[col != object$rm]
    new_data[, col_names[[i]]] <-
      if_else(col == object$rm,
              object$rm,
              (col - mean(to_scale))/ sd(to_scale))
...
#' @rdname step_scale_except_zero
#' @param x A `step_scale_except_zero` object.
#' @export
#'
tidy.step_scale_except_zero <- function(x, ...){
  if(is_trained(x)) {
